...
$TemplateFile = $ArtifactStagingDirectory + '\WindowsVirtualMachine.json'
$TemplateParametersFile = $ArtifactStagingDirectory + '\WindowsVirtualMachine.parameters.json'
...
New-AzureRmDeployment -Name ((Get-ChildItem $TemplateFile).BaseName + '-' + ((Get-Date).ToUniversalTime()).ToString('MMdd-HHmm')) `
